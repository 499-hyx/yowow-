# Archived `lib/warm-start-seed.ts`

> 归档说明：本文件原位于 `lib/warm-start-seed.ts`。经引用检查，当前没有任何运行路径 import 它；当前 MVP 的正式今日结果来自 `scripts/ingest.py` 安装的 `data/today/`。保留此文件仅用于追溯早期暖启动静态样板。

```ts
// 历史暖启动 · 静态示例内容种子（博士决策 B：先放少量静态 JSON 让 owner 进来给反馈）。
// 这是「少量静态 JSON」——不接 LLM、不查 Turso，让 /today 与引导最后一屏当场有真内容可看可评。
// 内容取自已验证的 petfood 样板（skills/adaptation-engine/replays/petfood-*）。
// 当前文件驱动 MVP 下，正式今日结果来自 scripts/ingest.py；本文件只保留历史样板。
