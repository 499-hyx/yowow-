# Archived `lib/engine-bridge.ts`

> 归档说明：本文件原位于 `lib/engine-bridge.ts`。经引用检查，当前没有任何运行路径 import 它；当前 MVP 是文件驱动，正式今日结果来自 `scripts/ingest.py` 安装的 `data/today/`。保留此文件仅用于历史追溯，不要在这里继续开发在线生成。

```ts
// ⚠️ 死代码 / DEAD CODE（2026-06-24 标注）：本文件当前【无任何 import】，不在任何运行路径上。
//    线上今日推荐是文件驱动：data/today 由 scripts/ingest.py 安装，/api/today 只读文件，不调本文件；
//    /api/regenerate 已停用。保留仅作历史参考——勿在此继续开发。启用在线生成须单独立工单。
//    详见 docs/CODE-MAP.md 的历史 / 预览 / 谨慎路径。
//
